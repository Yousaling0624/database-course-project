...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`fixed top-4 right-4 z-50 flex items-center px-4 py-3 rounded-lg shadow-lg transform transition-all duration-300 animate-in slide-in-from-right ${
...
export default function App() {
  ...
  const filteredData = data.filter(item => 
    item.name.includes(searchQuery) || item.code.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const handleAddMedicine = () => {
    ...
    const newItem = {
      id: Math.random(),
      code: `M${Math.floor(Math.random() * 9000) + 1000}`,
      name: formData.name,
      type: 'OTC', // 默认
      spec: '暂无规格',
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center p-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors group">
            ...
            <div className="ml-3 flex-1 overflow-hidden">
              <p className="text-sm font-medium text-slate-700 truncate group-hover:text-teal-700">Alex Chen</p>
              <p className="text-xs text-slate-400 truncate">销售总监</p>
            </div>
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <StatCard title="库存总量" value="12,580" trend="+12%" icon={Pill} color="blue" />
              <StatCard title="本月销售" value="¥ 89,200" trend="+5.4%" icon={ShoppingCart} color="emerald" />
              <StatCard title="临期预警" value="12" trend="需要处理" icon={AlertCircle} color="amber" isAlert />
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                      ...
                      <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            item.type === 'OTC' ? 'bg-emerald-100 text-emerald-800' : 'bg-blue-100 text-blue-800'
                          }`}>
                            {item.type}
                          </span>
                        </td>
              ...
              <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between text-sm text-slate-500">
                <span>显示 {filteredData.length} 条数据</span>
