...
export default function Sidebar({ onLogout, userRole, currentUser }) {
    ...
    return (
        <aside className="w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0 z-20">
            ...
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                <SidebarItem icon={LayoutDashboard} label="数据概览" to="/" />
                <SidebarItem icon={Pill} label="药品库存" to="/inventory" />
                <SidebarItem icon={Package} label="进货管理" to="/inbound" />
                <SidebarItem icon={ShoppingCart} label="销售订单" to="/sales" />

                <div className="pt-4 pb-1 pl-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">基础数据</div>

                {/* Admin only: 员工管理 */}
                {isAdmin && <SidebarItem icon={Users} label="员工管理" to="/users" />}

                <SidebarItem icon={Activity} label="客户管理" to="/customers" />
                <SidebarItem icon={Truck} label="供应商管理" to="/suppliers" />

                <div className="pt-4 pb-1 pl-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">报表与系统</div>
                <SidebarItem icon={BarChart3} label="报表中心" to="/reports" />

                {/* Admin only: 系统维护 */}
                {isAdmin && <SidebarItem icon={Settings} label="系统维护" to="/system" />}
            </nav>
