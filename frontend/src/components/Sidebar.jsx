...
export default function Sidebar({ onLogout, userRole, currentUser, isOpen, onClose }) {
    ...
    return (
        <>
            ...
            `}>
                <div className="h-16 flex items-center px-6 border-b border-slate-100 justify-between bg-white/50 backdrop-blur-sm sticky top-0 z-10">
                    <div className="flex items-center">
                        <div className="w-8 h-8 bg-gradient-to-br from-teal-500 to-emerald-600 text-white rounded-lg flex items-center justify-center mr-3 shadow-lg shadow-teal-500/20">
                            <Pill size={18} />
                        </div>
                        <span className="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-800 to-slate-600 tracking-tight">康源医药</span>
                    </div>
                    {/* Mobile Close Button */}
                    <button onClick={onClose} className="md:hidden text-slate-400 hover:text-slate-600 transition-transform hover:rotate-90">
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200">
                    ...
                    <div className="pt-6 pb-2 pl-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">基础数据</div>
                    ...
                    <div className="pt-6 pb-2 pl-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">报表与系统</div>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                    <div className="flex items-center p-2 rounded-xl hover:bg-white hover:shadow-md cursor-pointer transition-all duration-300 group ring-1 ring-transparent hover:ring-slate-100">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center shadow-inner ${isAdmin ? 'bg-amber-100' : 'bg-blue-100'}`}>
                        ...
                        <div className="ml-3 flex-1 overflow-hidden">
                            ...
                            <p className="text-xs text-slate-400 truncate flex items-center gap-1">
                                {isAdmin ? (
                                    <span className="text-amber-600 font-medium bg-amber-50 px-1.5 py-0.5 rounded text-[10px]">管理员</span>
                                ) : (
                                    <span className="text-blue-600 font-medium bg-blue-50 px-1.5 py-0.5 rounded text-[10px]">普通员工</span>
                                )}
                            </p>
                        </div>
                        <LogOut size={16} className="text-slate-400 hover:text-red-500 hover:scale-110 transition-all cursor-pointer" onClick={onLogout} />
                    </div>
                </div>
            </aside>
        </>
